...
# This ensures terraform.tfvars matches the production infrastructure
#
# Usage: ./scripts/sync-terraform-config.sh
#
...
# apply Terraform with default (dev) values against production infrastructure.
...
# Fetch all terraform-config parameters
...
if [ -z "$ENVIRONMENT" ]; then
  ...
  echo "Run 'aws ssm put-parameter --name $SSM_PREFIX/environment --value prod --type String' to create it"
  exit 1
fi
...
# Update the SSM parameters and re-run: ./scripts/sync-terraform-config.sh
...
echo "─────────────────────────────────────"
echo ""
echo "✓ Terraform config synced from SSM"
